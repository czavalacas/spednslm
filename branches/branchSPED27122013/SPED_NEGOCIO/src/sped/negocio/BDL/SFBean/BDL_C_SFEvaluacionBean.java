...
public class BDL_C_SFEvaluacionBean implements BDL_C_SFEvaluacionRemoto, 
    ...
    public List<Evaluacion> getPlanificaciones_BDL_WS(int nidRol,
        ...
        try {
            ...
            // qlString = qlString.concat(" AND CAST(e.startDate AS date) = CURRENT_DATE  ");
            ...
            if (beanFEva != null) {
                ...
                if (beanFEva.getEstadoEvaluacion() != null) {
                    ...
                    if (estado.compareTo("EJECUTADO") == 0 || estado.compareTo("PENDIENTE") == 0) {
                        ...
                        strQuery =
                            strQuery.concat(" AND (upper(eva.estadoEvaluacion) = :estado AND eva.comentarioEvaluador != NULL) ");
                    } else {
...
=======
            if (tipoBusqueda == 1) {
                strQuery2 =
                    ...
                    " AND eva.nidEvaluador = id AND eva.estadoEvaluacion = 'EJECUTADO' ) , " +
                    "(SELECT COUNT(DISTINCT eva) " + strQuery +
                    " AND eva.nidEvaluador = id AND eva.estadoEvaluacion = 'PENDIENTE' ) , " +
                    "(SELECT COUNT(DISTINCT eva) " + strQuery +
