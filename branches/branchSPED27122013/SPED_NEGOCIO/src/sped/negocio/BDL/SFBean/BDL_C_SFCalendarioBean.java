...
public class BDL_C_SFCalendarioBean implements BDL_C_SFCalendarioRemote,
    ...
    public List<BeanCalendario> getCalendarioActivo_BDL(int mesNumero) {
        try {
            ...
            return em.createQuery(qlString).setParameter("mesNumero",mesNumero).getResultList();
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public List<Object[]> getCalendarioActivoByUsuario_BDL(int mesNumero,int nidUsuario){
        try {
            String qlString = "SELECT c.cuartil,c.descripcionDia,c.diaNombre,c.dia,c.diaSemana,c.esDiaSemana,c.esFeriado \n" + 
                              ...
                              "FROM calendario c " +
                              "WHERE c.estado = '1' AND c.mes = "+mesNumero+" AND NOT EXISTS (SELECT c1.pk_fecha \n" + 
                              "								      FROM calendario c1,\n" + 
                              "									   addusca uc,   \n" + 
                              "									   admusua u     \n" + 
                              ...
                              "AND   c1.mes = ?                   \n "+
                              "ORDER BY 12 ASC ";
            return em.createNativeQuery(qlString).setParameter(1,nidUsuario).setParameter(2,nidUsuario)
                                                 .setParameter(3,mesNumero).getResultList();
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
